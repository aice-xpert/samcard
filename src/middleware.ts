...
async function checkSession(token: string, isBearer: boolean): Promise<boolean | null> {
    ...
    try {
        ...
        // Only cache negative results — a "false" is cheap to trust and reuse.
        // Never cache "true", so a deletion/logout takes effect on the very
        // next navigation instead of being masked by a stale positive entry.
        if (!valid) {
            sessionCache.set(token, { valid: false, expiry: Date.now() + CACHE_TTL_MS });
        } else {
