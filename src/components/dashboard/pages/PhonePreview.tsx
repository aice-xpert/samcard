...
function PhonePreviewComponent({
  ...
  const PLogoCircle = () => (
    <div style={{ width: 44, height: 44, borderRadius: '50%', background: '#fff', border: `2px solid ${T.green}`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 10px rgba(0,0,0,0.2)' }}>
      {hasBrandLogo ? (
        <img src={brandLogo} alt="Brand" style={{ width: 30, height: 30, borderRadius: '50%', objectFit: 'contain' }} />
      ) : (
        <div style={{ width: 32, height: 32, borderRadius: '50%', background: T.green, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 16 }}>
  ...
  // ─────────────────────────────────────────────────────────────────
  // Profile hero renderer (moved into a function to be called inline)
  // This contains all hero layouts, below-photo logo, tagline, and contact grid.
  // ─────────────────────────────────────────────────────────────────
  ...
  // Determine if Profile is the first item in the order
  const firstItem = renderItems[0];
  const profileIsFirst = firstItem === 'profile' || (firstItem === undefined && profileEnabled);
            ...
            <div className="rounded-[2.8rem] p-[3px] bg-muted/80 shadow-[0_0_0_1px_rgba(255,255,255,0.06),0_28px_70px_rgba(0,0,0,0.95)]">
              <div className="rounded-[2.6rem] overflow-hidden" style={{ background: T.phoneBgStyle || T.bg, overflowX: 'hidden' }}>
                ...
                <div data-scrollid={uid} className="overflow-y-auto"
                  ...
                  {/* Render Profile hero at top only if it is the first item */}
                  {profileIsFirst && renderProfileHero()}

                  {/* Render all sections in order (including Profile if not first) */}
                  {renderItems.map((id) => {
